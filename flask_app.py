...
#                 # Ensure the name is clean (no file extension)
...
#                     known_face_names.append(name)  # Add cleaned name
...
# attendance_lock = Lock()
# face_data_lock = Lock()  # Lock for face encoding data
# executor = ThreadPoolExecutor(max_workers=3)
...
#         for face_encoding in face_encodings:
#             face_distances = face_recognition.face_distance(known_face_encodings, face_encoding)
#             confidence_threshold = 0.6  # Adjusted threshold for accuracy
#             best_match_index = np.argmin(face_distances)
...
#     # Reload face encodings after adding new user
#     load_known_faces()
